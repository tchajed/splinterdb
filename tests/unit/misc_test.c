...
CTEST_SETUP(misc)
{
   // Tests in this file exercise log-printing using real-looking error
   // messages. It can be surprising to see error messages printed from a
   // successful test run. So lets send those messages to /dev/null unless
   // VERBOSE=1.
   if (Ctest_verbosity) {
      data->log_output = stdout;
      CTEST_LOG_INFO("\nVerbose mode on, successful tests may print messages "
